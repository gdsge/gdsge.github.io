...
algorithm can be implemented with the toolbox, by transforming the optimization problem to the first order condition and complementarity slackness condition.
...
One just needs to update the equilibrium object manually. In Krusell and Smith (1998), the equilibrium objects are the aggregate state transition coefficients.

The model can be solved within around 100 lines of GDSGE code plus 100 lines of MATLAB code.
...
:download:`ks1998.gmod <ks1998.gmod>` (you also need the file for grid points from Krusell and Smith (1998) :download:`points.kgd <points.kgd>`)
