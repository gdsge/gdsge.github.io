...
As can be seen, the implicit law of motion for the endogenous state :math:`\omega'` is captured by the consistency equation
...
Since the original problem's borrowing constraint is proportional to the aggregate endowment, we can use a transformation 
