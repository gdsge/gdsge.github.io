...
not necessarily  a boxed constraint. See example Cao and Nie (2018), which provides a global solution to the Kiyotaki-Moore model, where
...
See the toolbox API for details.
...
properly. A candidate that delivers good theoretical property and proves to be numerically robust is to start from a last-period problem,
so the algorithm can be viewed as taking the limit of a finite-horizon iterations.

The last-period problem has been so far trivial in the RBC and Heaton and Lucas examples, but turns out to be more complex
