**************************************************************************************
Guerrieri et al. (2020): Negative Supply Shocks That Produce Demand Shortages
**************************************************************************************
...
We following closely the notation in `Guerrieri et al. (2020) <http://economics.mit.edu/files/19351>`_. The total population is normalized to one, with a fraction
...
of workers in sector 1 follows a two-point Markov process with state in :math:`\{1,2\}`, where :math:`1` corresponds to normal times and :math:`2` corresponds to pandemics. During normal times, their labor endowment is :math:`\tilde{n}=\bar{n}`,
while when a supply shock hits, their labor endowment drops to :math:`\tilde{n}=\delta\bar{n}`
...
As in `Guerrieri et al. (2020) <http://economics.mit.edu/files/19351>`_, here
...
With the files returned by a local or `remote compiler <www.gdsge.com>`_, we can run the *iter_GLSW* file to solve the policy iterations,
and then pass the converged solutions to the *simulate_GLSW* file to simulate the ergodic set of the model.

.. image:: figures/policy.png
    :scale: 90 %

Starting from the model's ergodic set, we can simulate the *generalized impulse response functions* by comparing two sets of sample paths: one benchmark, 
the other with the *pandemic* shock hitting at the first period and the transition of the exogenous shock turned on.

.. image:: figures/irf_r_a1.png
    :scale: 30 %

As shown, after the economy is hit by a pandemic shock (a sector-specific disaster supply shock), the interest rate declines, suggesting that the induced demand shortages outweighs
the initial negative supply shock, a result highlighted by `Guerrieri et al. (2020) <http://economics.mit.edu/files/19351>`_.
Also as discussed in the paper, the strength of the induced demand shortages hinges on the magnitude of the *intra-temporal* elasticity of substitution.
To illustrate this, the codes generated by the gmod file can be reused to solve models with different values of parameter :math:`\rho`, which produces
the following.
