...
One can add the following line in the gmod file (recompilation needed) to switch to interpolating policy and state transition functions in the simulation.
...
However, as shown in the figure, the resulting ergodic distribution exhibits some artificial modes compared to the one obtained from solving the equilibrium system accurately at each simulation step.
